...
public class ForumService_Test {
    ...
    public void Test_Create_When_Set_Name_Empty_String_Then_Return_Null() {
        Forum_Section? newSection = Forum_Section.Create("", Guid.Parse("8fa2c964-c7e7-45f9-af89-acb914efc7d8"));
    ...
    public void Test_Create_When_Set_Master_Empty_Guid_Then_Return_Null() {
        Forum_Section? newSection = Forum_Section.Create("test1", Guid.Empty);
        Assert.Null(newSection);
    }

    [Fact(DisplayName = "更新Section时，正常完成")]
    public void Test_UpdateSection() {
        var oldSection = Forum_Section.Create("Old_Name", Guid.NewGuid(), "Old_Statement");
        if (oldSection is null) {
            return;
        }
        _outputHelper.WriteLine(JsonSerializer.Serialize(oldSection));
        ...
        var newSection = oldSection.UpdateSection(updateRequest);
        _outputHelper.WriteLine(JsonSerializer.Serialize(newSection));

        var res = oldSection.Id == newSection.Id &&
            newSection.Name == "New_Name" &&
            newSection.MasterId == newMasterId &&
            newSection.Statement == "New_Statement";
        Assert.True(res);
    }
    ...
    public void Test_UpdateSection_When_Id_Not_Equal_Then_No_Changes() {
        ...
        var oldMasterId = oldSection.MasterId;
        _outputHelper.WriteLine(JsonSerializer.Serialize(oldSection));
        ...
        var res = oldSection.Id == newSection.Id &&
            newSection.Name == "Old_Name" &&
            newSection.MasterId == oldMasterId &&
            newSection.Statement == "Old_Statement";
        Assert.True(res);
    }
    ...
    public void Test_CreateTopic_When_Right_Argument_Then_Return_Forum_Topic() {
        ...
        var res = _forumDomainService.CreateTopic(request);
